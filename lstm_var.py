...
def analyze_portfolio_var(returns_df, weights, sequence_length=21, backtest_days=378,
    ...
    """
    Analyze portfolio with rolling VaR, ES, and LSTM model predictions, including breach calculations.
    """
    # Calculate portfolio returns
    ...
    # Calculate historical VaR and ES for each day in backtest period
    hist_vars = []
    hist_es = []
    
    # Get the full returns series
    all_returns = portfolio_returns
    
    # For each day in backtest period
    for t in range(backtest_days):
        # Calculate the correct window indices
        # end_idx points to the day right before the current backtest day
        end_idx = len(all_returns) - backtest_days + t
        start_idx = end_idx - rolling_window
        
        # Get the window of historical returns
        window_returns = all_returns[start_idx:end_idx]
        
        # Calculate VaR for this specific day
        var = np.percentile(window_returns, (1 - confidence_level) * 100)
        hist_vars.append(var)
        
        # Calculate ES
        losses = window_returns[window_returns <= var]
        es = np.mean(losses) if len(losses) > 0 else var
        hist_es.append(es)
    ...
    # Adjust hist_vars and hist_es to align with LSTM outputs
    hist_vars = hist_vars[sequence_length - 1:]
    hist_es = hist_es[sequence_length - 1:]
    ...
    with torch.no_grad():
        X_test_tensor = torch.tensor(X_test, dtype=torch.float32).to(device)
        pred_mean, pred_vol = model(X_test_tensor)
        
        # Unscale predictions
        pred_mean = pred_mean.cpu().numpy().flatten()
        pred_vol = pred_vol.cpu().numpy().flatten()
        
        predicted_returns = scaler.inverse_transform(pred_mean.reshape(-1, 1)).flatten()
        predicted_vols = pred_vol * scaler.scale_
        
        # Calculate LSTM-based VaR directly from predictions
        lstm_vars = predicted_returns - (stats.norm.ppf(confidence_level) * predicted_vols)
        
        # Calculate LSTM-based ES directly from predictions
        z_score = stats.norm.ppf(confidence_level)
        pdf_z = stats.norm.pdf(z_score)
        cdf_z = confidence_level
        es_factor = pdf_z / (1 - cdf_z)
        lstm_es = predicted_returns - (predicted_vols * es_factor)
    
    # Align `aligned_returns` to match the length of LSTM predictions
    aligned_returns = all_returns[-len(lstm_es):]
    
    # Calculate breaches
    hist_es_breaches = aligned_returns < hist_es[:len(aligned_returns)]
    lstm_es_breaches = aligned_returns < lstm_es
...
def analyze_covid_stress_period(returns_df, weights, market_event_start='2020-02-18', 
                              ...
                              confidence_level=0.95, sequence_length=21):
    """
    Analyze portfolio behavior during COVID-19 stress period with maximum available historical data
    """
    ...
    for i, current_date in enumerate(analysis_dates):
        ...
        with torch.no_grad():
            pred_mean, pred_vol = model(X_pred_tensor)
            pred_mean = pred_mean.cpu().numpy().flatten()[0]
            pred_vol = pred_vol.cpu().numpy().flatten()[0]
        
        # Unscale predictions
        pred_mean_unscaled = scaler.inverse_transform([[pred_mean]])[0, 0]
        pred_vol_unscaled = pred_vol * scaler.scale_[0]
        
        # Calculate LSTM VaR and ES
        lstm_var = pred_mean_unscaled - (stats.norm.ppf(confidence_level) * pred_vol_unscaled)
        z_score = stats.norm.ppf(confidence_level)
        pdf_z = stats.norm.pdf(z_score)
        cdf_z = confidence_level
        es_factor = pdf_z / (1 - cdf_z)
        lstm_es = pred_mean_unscaled - (pred_vol_unscaled * es_factor)
        ...
        results.append({
            ...
            'LSTM_VaR': lstm_var,
            'LSTM_ES': lstm_es,
            'LSTM_Predicted_Returns': pred_mean_unscaled,
            'LSTM_Predicted_Volatility': pred_vol_unscaled,
...
def main():
    ...
    for i, weights in enumerate(portfolios):
        ...
        predicted_returns, hist_vars, hist_es, lstm_vars, lstm_es, hist_es_breaches, lstm_es_breaches = analyze_portfolio_var(
            returns_df, weights, backtest_days=backtest_days,
